...
(:action place-block
    ...
    (and
        (height ?bpos ?hbefore)
        (SUCC ?hafter ?hbefore)
        (or (has_color ?bpos ?c)(and (has_color ?bpos white)(= ?hbefore n0)))
        (has_block ?c)
...
(:action remove-block
    :parameters (?bpos - position ?n1 - position ?hbefore - numb ?hafter - numb ?c - color)
    :precondition
    (and
        (height ?bpos ?hbefore)
        (SUCC ?hbefore ?hafter)
        (exists (?c1 - color)(not(has_block ?c1)))
